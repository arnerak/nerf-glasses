...
#include "ngp/ngp_common.cuh"
#include "ngp/testbed.cuh"
#include "ngp/thread_pool.h"
...
py::array_t<float> Testbed::render_to_cpu(int width, int height, int spp, bool linear) {
	...
	for (int i = 0; i < spp; ++i) {
		float start_alpha = ((float)i)/(float)spp;
		float end_alpha = ((float)i + 1.0f)/(float)spp;
...
PYBIND11_MODULE(pynmr, m) {
	m.doc() = "Combined nerf mesh renderer";
    ...
    // Enable transparent conversion between NumPy arrays and glm::vec3
    py::implicitly_convertible<py::array_t<float>, glm::vec3>();

	py::class_<GltfNode> gltfNode(m, "GltfNode");
	gltfNode
		.def_readwrite("scale", &GltfNode::scale)
		.def_readwrite("translation", &GltfNode::translation)
		;

	py::class_<GltfScene> gltfScene(m, "GltfScene");
	gltfScene
		.def_readonly("nodes", &GltfScene::nodes)
		;

	py::class_<NerfMeshRenderer> nmr(m, "NerfMeshRenderer");
	nmr
		.def(py::init<int, int>(), py::arg("width"), py::arg("height"))
		.def("frame", &NerfMeshRenderer::frame, py::call_guard<py::gil_scoped_release>(), "Process a single frame. Renders if a window was previously created.")
		.def("load_mesh", &NerfMeshRenderer::loadMesh, 
			py::arg("path"), 
			py::arg("t") = Eigen::Vector3f{ 0.f, 0.f, 0.f }, 
			py::arg("s") = Eigen::Vector3f{ 1.f, 1.f, 1.f }, 
			py::arg("r") = Eigen::Vector4f{ 0.f, 0.f, 0.f, 1.f },
			py::keep_alive<1, 0>(), "Load a gltf mesh")
		.def("load_nerf", &NerfMeshRenderer::loadNerf, py::arg("path"), py::keep_alive<1, 0>(), "Load a previously saved snapshot")
		.def("remove_floaties", &NerfMeshRenderer::removeFloaties, "Remove floater artifacts")
		.def("orbit", &NerfMeshRenderer::orbit, py::arg("delta_azimuth"), py::arg("delta_polar"), py::arg("delta_zoom"), "Orbit camera around pivot point")
		.def_readwrite("view_projection_mat", &NerfMeshRenderer::viewProjectionMat)
		;
